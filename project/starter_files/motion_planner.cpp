...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  auto yaw = goal_state.rotation.yaw + M_PI / 2.0;  // <- Fix This
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    goal_offset.location.x += std::cos(yaw) * offset;  // <- Fix This
    goal_offset.location.y += std::sin(yaw) * offset;  // <- Fix This

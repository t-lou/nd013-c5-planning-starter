...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  const double time = _lookahead_time;
  const double vel_end = velocity_mag + accel_mag * time;
  auto look_ahead_distance = (velocity_mag + vel_end) * time / 2.0;  // <- Fix This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      const double distance = get_look_ahead_distance(ego_state) - _stop_line_buffer;
      goal.location.x += distance * std::cos(ang);  // <- Fix This
      goal.location.y += distance * std::sin(ang);  // <- Fix This
    ...
    // const bool cond = utils::magnitude(ego_state.velocity) <= _stop_threshold_speed;
    const bool cond = distance_to_stop_sign <= P_STOP_THRESHOLD_DISTANCE;
    if (cond) {  // -> Fix this
